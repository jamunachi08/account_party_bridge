...
def on_account_update(doc, method=None):
	...
	if not cfg or not cfg.sync_on_rename:
		return
...
def apply_map(account, cfg):
	...
	"""
	if account.is_group:
		if not cfg.create_group_node or not cfg.group_doctype:
			return False
		return create_group_node(account, cfg)

	if not cfg.create_leaf_party or not cfg.party_doctype:
		return False
...
def build_party_name(account, cfg):
	...
	if cfg.append_company_abbr and account.company:
